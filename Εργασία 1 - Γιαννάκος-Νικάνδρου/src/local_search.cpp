...
bool add_steiner_point_local_search(CDT &cdt, const CDT::Edge &edge, const std::vector<Point_2> &region_points)
{
    ...
    {
        std::cerr << "Error calculating midpoint: " << e.what() << std::endl;
    }
    ...
    {
        std::cerr << "Error calculating projection: " << e.what() << std::endl;
    }
    ...
    for (int i = 0; i < candidate_points.size(); i++)
    {
        ...
        // if (is_point_inside_constraints(candidate_points[i], region_boundary))
        // {
        //     std::cout << "The point is inside the region!" << std::endl;
        // }
        // else
        // {
        //     std::cout << "The point is outside the region!" << std::endl;
        // }
    ...
    if (!st_contenders.empty())
    {
        ...
        cout << "Best Steiner point added at: ("
...
{
    cout << "Starting Local Search part of the Algorithm" << endl;

    int max_no_of_iterations = method_parameters[0];
